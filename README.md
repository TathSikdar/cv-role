...
| `application-questions` | `/application-questions <slug> <url>`<br>+ the questions, pasted | Answers a form's free-text screening questions ("why us", "tell us about a time"). Short, human-sounding, and never a restatement of the CV: it reads the master CV and the CV's source projects for the story, and the rendered CV only for the numbers. No build step; markdown out. |
